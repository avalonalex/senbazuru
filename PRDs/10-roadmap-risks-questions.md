...
| 14 | A `fold and unfold`: one record or two, and its name | **Decided 2026-09-23**: one move, the *pre-crease*, printed `pre-crease`, whose one record of kind `Precrease` changes nothing but the new crease ([D12](decisions.md#d12-the-text-syntax)) | Two records, the fold and its unfold: a folded state no figure draws, and a pair every consumer has to match up | M2's runner |
