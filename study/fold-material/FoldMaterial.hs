...
import Senbazuru.Origami.Surface (MaterialMesh, MaterialSample, Mesh (..), Sample (..), Triangle, materialSample, materialU, materialV)
...
makeMesh :: Int -> FoldCase -> MaterialMesh
...
sharpMesh :: Int -> FoldCase -> MaterialMesh
...
meshOn :: Diagonals -> [Double] -> (Double -> Double -> V3) -> MaterialMesh
meshOn diagonals axis point =
  let count = length axis
      verts = [materialSample u v (point u v) | v <- axis, u <- axis]
      cell i j =
        let a = j * count + i
            b = a + 1
            c = a + count
...
resolvedTriangles :: MaterialMesh -> [(MaterialSample, MaterialSample, MaterialSample)]
...
componentCount :: MaterialMesh -> Int
