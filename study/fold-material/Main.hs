...
import BodyCreaseSeedGallery (writeBodyCreaseSeed)
import BodyDirectionGallery (writeBodyDirection, writeBodyPairs, writeBodyThird)
import BodyFeasibilityGallery (writeBodyFeasibility)
...
import BodyGuardedInspection (writeBodyGuardedInspection)
import BodyInitializationGallery (writeBodyInitialization)
import BodyOverlapRestorationGallery (writeBodyOverlapRestoration)
import BodyPairInspectionGallery (writeBodyPairInspection)
import BodyPatchCheckpointGallery (writeBodyPatchCheckpoints)
import BodyPatchGallery (writeBodyPatch)
import BodyPatchSubdivisionGallery (writeBodyPatchSubdivision)
import BodyPlaneGuardGallery (writeBodyPlaneGuard)
import BodyPlaneLossGallery (writeBodyPlaneLoss)
import BodyRestorationGallery (writeBodyRestoration, writeFreshRestoration)
import BodyRestorationLoop (writeBodyRestorationLoop)
import BodyVisibleLayersGallery (writeBodyVisibleLayers)
import BodyVisualToleranceGallery (writeBodyVisualTolerance)
...
import HeldLayoutCostsGallery (writeHeldLayoutCosts)
import HeldRefinementGallery (writeHeldRefinement)
...
import PrescribedBendGallery (writePrescribedBends)
...
import TransitionRefinementGallery (writeTransitionRefinement, writeWholeBendRefinement)
...
main = do
  args <- getArgs
  case args of
    ["--body-visible-layers", source, destination] -> writeBodyVisibleLayers source destination
    ["--body-visual-tolerances", source, destination] -> writeBodyVisualTolerance source destination
    ...
    ["--body-initialization", source, destination] -> writeBodyInitialization source destination
    ["--body-overlap-restoration", source, destination] -> writeBodyOverlapRestoration source destination
    ["--body-contact-14-55", source, destination] -> writeBodyGuardedInspection source destination
    ...
    ["--body-fresh", source, destination] -> writeBodyFresh source destination
    ["--body-restoration", source, destination] -> writeBodyRestoration source destination
    ["--body-plane-loss", source, destination] -> writeBodyPlaneLoss source destination
    ["--body-plane", source, destination] -> writeBodyPlaneGuard source destination
    ["--body-final", source, destination] -> writeBodyFinalContact source destination
    ["--body-short", source, destination] -> writeBodyContinuation source destination
    ["--body-third", source, destination] -> writeBodyThird source destination
    ["--body-pairs", source, destination] -> writeBodyPairs source destination
    ["--body-direction", source, destination] -> writeBodyDirection source destination
    ["--body-geometry", source, destination] -> writeBodyGeometry source destination
    ["--body-replay", source, destination] -> writeBodyReplay source destination
    ["--body-contact", source, destination] -> writeBodyContact source destination
    ["--body-subdivision", source, destination] -> writeBodyPatchSubdivision source destination
    ["--body-checkpoints", source, destination] -> writeBodyPatchCheckpoints source destination
    ["--body-patch", destination] -> writeBodyPatch destination
    ["--held-layout-costs", source, destination] -> writeHeldLayoutCosts source destination
    ["--held-refinement", source, destination] -> writeHeldRefinement source destination
    ...
    ["--boundary-length", destination] -> writeBoundaryLength destination
    ["--boundary-solves", destination] -> writeBoundarySolves destination
    ...
    ["--band-refinement-8x2", destination] -> writeBandRefinement8x2 destination
    ["--band-refinement-8", destination] -> writeBandRefinement8 destination
