...
two opposing flaps. It also folds one third of the square onto the middle
third, lifts that two-layer flap through 90°, and lowers it without separating
the layers.
...
`prepareFlapToward` reads it from the paper that moves: each moving face beside
the hinge, which must lie flat and all on one side of the hinge line. It first
read the face held still instead, which made a page turn depend on which
crease was listed first, because a page's held faces lie on both sides.
...
endpoint, while each change alone leaves a different failure. The
[common-policy grid](notes/combined-crease-refinement.md) then accepts all ten
