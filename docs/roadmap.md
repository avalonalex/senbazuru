...
worsening gaps too. Next compare one matched direction with six extra overlap
guards for that pair. They are stricter than the existing geometry tolerance:
a no-worsening guard would also constrain the saved passing 1/128 direction.
...
1. **One checked flap rotation**, a bounded part of
   ...
   **Next: one matched direction adding the six 55–93 overlap guards at the
   same saved start, with all thirteen old guards and acceptance checks kept.**
