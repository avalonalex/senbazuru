...
| `Senbazuru.Sequence.Elaborate` | A `Checked` sequence → an `Elaborated` one, in the *core moves* a run performs: each `let` replaced by what it names wherever it is used, and each [pre-crease](glossary.md#origami) made one core move, `CorePrecrease`, since what it leaves is a crease and not two motions. Every core move keeps an `Origin`, the move the author wrote and its span, so a refusal from a run can still quote and point at the author's text. Knows no paper. |
