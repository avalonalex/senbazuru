...
| `frog-sequence.svg` | Five checked frog-base milestones at one scale, generated by the production step renderer |
| `bird-open.svg` | The first bird flap raised above its stationary packet, from checked sequence frame 5 |
...
The README's three-example gallery now includes the frog checkpoints and an
open bird flap, both drawn through the production surface interface:

```bash
stack run senbazuru-material-study -- --basic-bases build/fold-material
cp build/fold-material/frog-sequence-side-above.svg docs/img/frog-sequence.svg
stack run -- render examples/bird-base-sequence.fold --frame 5 --view iso \
  --width 420 --height 360 -o docs/img/bird-open.svg
```

The frog page is five checked milestones, not a continuous animation. The open
bird image is frame 5 of the checked sequence; no positions are interpolated.
The existing pinwheel underside remains to illustrate a circular layer order.
