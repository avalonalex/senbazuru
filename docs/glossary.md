...
| **Z-fighting** | What a 3D viewer shows where two faces lie at exactly the same depth: a shimmer of both, because its depth buffer has no way to say which is in front. The current glTF export separates flat layers for display; that spacing is not physical paper thickness. See [notes/connected-paper-surface.md](notes/connected-paper-surface.md). |
